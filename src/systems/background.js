  ...
  const rebuild = () => {
    ...
    for (const layer of LAYERS) {
      ...
      for (let index = -1; index <= Math.ceil(k.width() / imageWidth) + 1; index += 1) {
        const image = k.add([
          k.sprite(layer.name), k.pos(0, 0), k.scale(scale), k.fixed(), k.z(layer.z),
        ]);
        image.onUpdate(() => {
          const offset = ((cameraX() * layer.factor * (k.height() / VIEW_HEIGHT)) % imageWidth + imageWidth) % imageWidth;
          image.pos.x = index * imageWidth - offset;
        });
        images.push(image);
      }
    }
