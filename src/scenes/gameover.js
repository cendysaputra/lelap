...
import {
  ...
  GAME_OVER_ART_Y, GAME_OVER_MENU_Y, GAME_OVER_PANEL_MARGIN,
  GAME_OVER_PANEL_SCALE, GAME_OVER_RETRY_Y, GAME_OVER_TITLE_Y, VIEW_HEIGHT,
} from "../config.js";
...
export function registerGameOverScene(k) {
  k.scene("gameover", ({ reason = "TERTANGKAP" } = {}) => {
    ...
    const retry = createButton(k, { label: "ULANG", pos: k.vec2(k.width() / 2, k.height() * GAME_OVER_RETRY_Y), z: 90, onPress: () => fadeTo(k, "game") });
    const menu = createButton(k, { label: "MENU", pos: k.vec2(k.width() / 2, k.height() * GAME_OVER_MENU_Y), z: 90, onPress: () => fadeTo(k, "menu") });
