import {
  PEEKER_RANGE, PEEKER_RETURN_SPEED, PEEKER_SPEED, PEEKER_RETURN_DELAY,
} from "../config.js";
...
export function createPeeker(k, position) {
  const ghost = k.add([
    ...
    { dangerous: true, origin: position.clone(), facing: 1, moving: false, hiddenTime: 0 },
  ]);
  const visual = ghost.add([
    hasAsset("hantu/pengintai/diam") ? k.sprite("hantu/pengintai/diam") : k.rect(224, 448),
    k.anchor("bot"), k.scale(0.25), { currentFrame: "hantu/pengintai/diam" },
  ]);
  setVisualFrame(k, visual, "hantu/pengintai/diam");
  ...
  ghost.onUpdate(() => {
    ...
    ghost.hiddenTime = player.isHidden ? ghost.hiddenTime + k.dt() : 0;
    ghost.facing = Math.sign(dx) || ghost.facing;
    ghost.moving = false;
    if (player.isHidden) {
      const delta = ghost.origin.sub(ghost.pos);
      if (ghost.hiddenTime >= PEEKER_RETURN_DELAY && delta.len() > 2) {
        ghost.facing = Math.sign(delta.x) || ghost.facing;
        const next = ghost.pos.add(delta.unit().scale(Math.min(delta.len(), PEEKER_RETURN_SPEED * k.dt())));
        const movement = moveOutsideLamp(k, ghost, next);
        ghost.pos = movement.position;
        ghost.moving = !movement.blocked;
      }
    } else if (player.pos.dist(ghost.pos) <= PEEKER_RANGE && !playerFacesGhost) {
