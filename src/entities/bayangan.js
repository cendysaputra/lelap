...
export function createShadow(k, position) {
  ...
  const visual = ghost.add([
    hasAsset("hantu/bayangan/sembunyi") ? k.sprite("hantu/bayangan/sembunyi") : k.rect(56, 24),
  ...
  ghost.onUpdate(() => {
    ...
    ghost.timer += k.dt();
    if (ghost.state === "hidden" && Math.abs(player.pos.x - ghost.pos.x) <= SHADOW_TRIGGER_RANGE) {
      ghost.state = "warn";
      ghost.timer = 0;
